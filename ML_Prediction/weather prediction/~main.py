...
feature_names = ['Humidity', 'Wind Speed (km/h)', 'Wind Bearing (degrees)','Visibility (km)', 'Pressure (millibars)'] # Adjust feature names as needed

# Create a function to get user inputs and make predictions
def get_data_and_predict(loaded_model,user_data):
...
def get_weather():
    ...
    for feature in feature_names:
        ...
        if feature=='Wind Speed (km/h)':
            user_input = wind*3.6
        if feature=='Wind Bearing (degrees)':
            user_input = winddir
        if feature=='Visibility (km)':
            user_input = visibility
        if feature=='Pressure (millibars)':
            user_input = pressure
    ...
    data=(" \n---------------------------------------------------------------" +
          ...
          f"Sunset: {sun(sunset)}\n\n" + f"{descp} : " +f"{description}\n"
          f"Pressure : {pressure} hPa\n" +
          f"{cityname}, {fullc_n}\n" + f"{time}" +
          f"\nModel Prediction : {get_data_and_predict(loaded_model,user_data)}\n" +
          "-----------------------------------------------------------------\n")
    print(data)
